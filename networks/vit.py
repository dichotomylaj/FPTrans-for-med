...
class DFN(nn.Module):
    """Domain Feature Navigator adapter for ViT blocks.
    1x1 conv bottleneck with residual connection, operates on [B, N, C] tensors.
    Zero-init of final BN ensures identity start.
    """
    def __init__(self, channels, reduction=4, dropout=0.0, scale=1.0, zero_init=True):
        super().__init__()
        ...
        if zero_init:
            last_ln = self.adapter[-1]
            nn.init.zeros_(last_ln.weight)
            nn.init.zeros_(last_ln.bias)
...
class Block(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
                 ...
                 use_dfn=False, dfn_reduction=4, dfn_scale=1.0):
        super().__init__()
        ...
        # DFN adapter (inserted after MLP, before residual addition)
        self.use_dfn = use_dfn
        if use_dfn:
            self.dfn = DFN(dim, reduction=dfn_reduction, scale=dfn_scale, zero_init=True)

    def forward(self, x):
        x = x + self.drop_path(self.attn(self.norm1(x)))
        mlp_out = self.drop_path(self.mlp(self.norm2(x)))
        if self.use_dfn:
            mlp_out = self.dfn(mlp_out)
        x = x + mlp_out
        return x
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        if self.allow_mod:
            ncls = 15 if opt.dataset in ["PASCAL", "ORGANT2"] else 60
            if opt.coco2pascal:
                ncls = 60
        ...
        # DFN: enable adapter in last N blocks for domain decoupling
        num_dfn_blocks = 0
        if opt is not None and getattr(opt, 'DFN', False):
            num_dfn_blocks = getattr(opt, 'dfn_num_blocks', 4)
            _logger.info(' ' * 5 + f'==> DFN enabled in last {num_dfn_blocks} ViT blocks '
                         f'(reduction={getattr(opt, "dfn_reduction", 4)}, '
                         f'scale={getattr(opt, "dfn_scale", 1.0)})')

        self.blocks = nn.Sequential(*[
            Block(
                dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, drop=drop_rate,
                attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer, act_layer=act_layer,
                use_dfn=(i >= depth - num_dfn_blocks),
                dfn_reduction=getattr(opt, 'dfn_reduction', 4) if opt is not None else 4,
                dfn_scale=getattr(opt, 'dfn_scale', 1.0) if opt is not None else 1.0,
            )
