...
DFN adapter experiment:

```bash
# Keep original FPTrans behavior
cuda 0 python run.py train with configs/organt2_vit.yml DFN=False

# Enable DFN during source-domain training
cuda 0 python run.py train with configs/organt2_vit.yml DFN=True

# Fine-tune only DFN parameters from a checkpoint
cuda 0 python run.py train with configs/organt2_vit.yml DFN=True dfn_freeze_base=True ckpt=/path/to/checkpoint.pth
```
