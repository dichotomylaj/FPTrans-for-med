...
def setup(ex):
    ...
    def ex_config():
        ...
        scale_min = 0.9
        scale_max = 1.1
        dataset = "PASCAL"              # str, dataset name. [PASCAL|COCO|ORGANT2]
        use_split_coco = True
        rotate = 10
        pad_type = 'reflect'            # str, pad type, reflect or constant

        # ORGANT2 3D medical data
        organt2_data_dir = "/home/liuyuhan/lianaijia/organt2"
        slab_k = 3                      # int, number of slices in a slab (must be odd, default 3 for 3-channel input)
        fold_id = 0                     # int, fold ID for 5-fold CV [0-4]
        test_fold_start = 0             # int, start index of test cases (inclusive)
        test_fold_end = 4               # int, end index of test cases (exclusive)
        val_fold_start = 0              # int, start index of val cases within train set (inclusive)
        val_fold_end = 4                # int, end index of val cases within train set (exclusive)
        ...
        # LCM configurations (Low-level Calibration Module from LoEC) ==================
        lcm = False                     # bool, enable LCM during testing
        lcm_K = 3                       # int, number of top-confidence patches to select
        lcm_w = 0.6                     # float, scaling factor for calibration signal
        lcm_beta = 0.7                  # float, bias term subtracted from similarity
        
        # Prototype refinement configurations
        proto_refine = False            # bool, enable test-time prototype refinement
        proto_refine_threshold = 0.9    # float, confidence threshold for selecting query patches
        proto_refine_alpha = 0.5        # float, blending weight for query prototype

        # Boundary propagation configurations
        boundary_prop = False            # bool, enable test-time boundary propagation
        boundary_fg_thresh = 0.8        # float, confidence threshold for foreground seeds
        boundary_bg_thresh = 0.8        # float, confidence threshold for background seeds
        boundary_temperature = 10.0     # float, temperature for similarity-to-vote softmax
        boundary_blend = 0.5            # float, blend weight for propagated vs original probs

        # DFN configurations (Domain Feature Navigator adapter in ViT deep blocks) ==============
        DFN = False                     # bool, enable DFN adapter in deep ViT blocks
        dfn_num_blocks = 4              # int, number of last ViT blocks to insert DFN into
        dfn_reduction = 4               # int, channel reduction ratio inside DFN bottleneck
        dfn_scale = 1.0                 # float, residual scale for DFN feature update
        dfn_freeze_base = False         # bool, train only DFN params (freeze backbone) when True
